<div class="relative z-10" x-data="{ showDropdown: @entangle('showDropdown').live }">

    <div class="relative mx-auto">
        <div class="relative">

            <input type="text" placeholder="{{__('Buscar...')}}" wire:model.live='s'
            ...
            <div class="absolute w-full mt-2 text-sm text-left bg-white border border-gray-300 rounded shadow">
                <ul class="w-full pb-2 space-y-2 overflow-y-auto scroll-pb-2 scroll-pt-11" id="options"
                    ...
                    @if ($this->users && $this->users->count())
                        <li>
                            <h2 class="bg-gray-100 px-4 py-2.5 text-xs font-semibold text-gray-900">{{__('Usuaris')}}</h2>
                            <ul class="text-sm text-gray-800">
                                @foreach($users as $user)
                                    <a wire:navigate href="{{ route('profile-page', $user) }}"
                    ...
                    @endif
                    @if ($this->projects && $this->projects->count())
                        <li>
                            <h2 class="bg-gray-100 px-4 py-2.5 text-xs font-semibold text-gray-900">{{__('Projectes')}}</h2>
                            <ul class="text-sm text-gray-800">
                                @foreach ($this->projects as $project)
                                    <a wire:navigate href="{{ route('post.show', [$project]) }}"
                                        class="block px-4 py-2 hover:bg-sky-600 hover:text-white" id="option-2"
                                        role="option" tabindex="-1">
                                        {{ $project->title }}</a>
                                @endforeach
                ...
                @if ($this->s && !count($this->projects) && !count($this->users))
                    ...
                    <div class="px-6 text-sm text-center border-t border-gray-100 py-14 sm:px-14">
                        ...
                        <p class="mt-4 font-semibold text-gray-900">{{__("No s'han trobat resultats")}}</p>
                        <p class="mt-2 text-gray-500">{{__('No hem trobat res amb aquest terme. Si us plau, torneu-ho a provar.')}}</p>
                    </div>
            </div>
            @endif
        </div>
