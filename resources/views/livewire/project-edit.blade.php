            ...
            <form wire:submit.prevent="save">
                <div class="p-8 bg-white">
                    <div class="space-y-6">
                        <div class="flex justify-between">
                            <h1 class="flex text-lg font-medium leading-6 text-gray-900">
                                <a href="{{ url()->previous() ?? route('home') }}" class="mr-4">
                                ...
                                {{__('Preferències del projecte')}}
                            </h1>
                            @if($post->id)
                                <button type="button" wire:confirm="{{__('Estàs segur que vols eliminar el projecte?')}}" wire:click="delete({{$post->id}})" class="bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-opacity-50">{{__('Eliminar')}}</button>
                            @endif
                        </div>
                        ...
                        <div>
                            <b>{{__('Nom')}}</b>
                            <x-text-input wire:model='title' class="mt-2" />
                        </div>

                        <div>
                            <b>{{__('Descripció')}}</b>
                            <div class="mt-2">
                                <x-rich-text-editor wire:model='body' />
                            </div>
                        </div>

                        <div>@include('partials.file-uploader')</div>
                                    ...
                                    <li class="flex items-center justify-between py-4 pl-4 pr-5 text-sm leading-6">
                                        <div class="flex items-center flex-1 w-0">
                                            ...
                                            <div class="flex flex-1 min-w-0 gap-2 ml-4">
                                                <input type="text"
                                                    wire:model='projectDocuments.{{ $index }}.name'
                                                    class="block w-full border-none">
                                            </div>
                                        </div>
                                        <div class="flex items-center ml-4">
                                            <a href="{{ \Storage::url($document['path']) }}" target="_blank"
                                                class="font-medium text-indigo-600 hover:text-indigo-500">{{__('Veure fitxer')}}</a>
                        ...
                        <div>
                            ...
                            @foreach($this->links as $index => $link)
                                <div class="flex align-center gap-6 mt-2">
                                    <input type="text"  wire:model="links.{{ $index }}.name" class="rounded-md w-1/3" placeholder="{{__("Nom")}}">
                                    <input type="text" wire:model="links.{{ $index }}.link" class="rounded-md w-2/3" placeholder="{{__("Link")}}">
                                    <button type="button" wire:click="removeLink({{ $index }})" class="text-red-500">
                    ...
                    <div class="w-1/3 flex flex-col gap-6 pl-10">
                        <div>
                            <fieldset>
                                <b>{{__('Categories del projecte')}}</b>
                                        ...
                                        <div class="relative flex items-start">
                                            ...
                                            <div class="ml-3 text-sm leading-6">
                                                <label for="category-{{ $category->id }}"
                                                    class="font-medium text-gray-900">{{ __($category->name) }}</label>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="mt-2 -space-y-px bg-white rounded-md shadow-sm isolate">
                                <label
                                    ...
                                    <span class="flex flex-col ml-3">
                                        <span class="block text-sm font-medium">{{ __('Accés públic') }}</span>
                                        <span class="block text-sm">{{ __('Aquest projecte estarà disponible per a tothom') }}</span>
                                    </span>
                                </label>
                                <label
                                    ...
                                    <input type="radio" wire:model='privacy' value="1"
                                        class="mt-0.5 h-4 w-4 shrink-0 cursor-pointer text-sky-600 border-gray-300 focus:ring-sky-500 active:ring-2 active:ring-offset-2 active:ring-sky-500">
                                    <span class="flex flex-col ml-3">
                                        <span class="block text-sm font-medium">{{ __('Accés privat') }}</span>
                                        <span class="block text-sm">{{ __('Aquest projecte estarà disponible només per tu') }}</span>
                                    </span>
                                </label>
                            </div>
                        </fieldset></div>
                        ...
                        <ul class="list-disc pl-5" id="filters-list">
                            <div class="filter-category hidden" id="filter-1">
                                @foreach (DB::table('filters')->where('category', 'projects')->select('id', 'title', 'filter_value')->get()->groupBy('title') as $title => $filters)
                                    <li>
                                        <div class="filter-title font-semibold mt-2">
                                            <span class="cursor-pointer" onclick="toggleDropdown('filter-values-{{ $loop->index }}', this)">
                                            {{ __($title) }} <span class="ml-2">&#9662;</span>
                                            </span>
                                        </div>
                                        <ul class="filter-options hidden pl-8 flex flex-wrap gap-2" id="filter-values-{{ $loop->index }}">
                                            @foreach ($filters as $filter)
                                                <li class="flex items-center">
                                                    <button type="button"
                                                        data-filter-id="{{ $filter->id }}"
                                                        wire:click="toggleFilter({{ $filter->id }})"
                                                        onclick="toggleFilterLocal({{ $filter->id }}, this, event)"
                                                        class="chip inline-flex items-center px-3 py-1 mr-2 mb-2 rounded-full border border-gray-300 text-sm cursor-pointer select-none hover:bg-gray-100 
                                                            {{ in_array($filter->id, $selectedFilters ?? []) ? 'bg-blue-500 text-white' : '' }}"
                                                    >
                                                        {{ __($filter->filter_value) }}
                                                    </button>
                                                </li>
                                            @endforeach
                                        </ul>
                            ...
                            <div class="filter-category hidden" id="filter-2">
                                @foreach (DB::table('filters')->where('category', 'backgrounds')->select('id', 'title', 'filter_value')->get()->groupBy('title') as $title => $filters)
                                    <li>
                                        <div class="filter-title font-semibold mt-2">
                                            <span class="cursor-pointer" onclick="toggleDropdown('filter-values-fondos-{{ $loop->index }}', this)">
                                            {{ __($title) }} <span class="ml-2">&#9662;</span>
                                            </span>
                                        </div>
                                        <ul class="filter-options hidden pl-8 flex flex-wrap gap-2" id="filter-values-fondos-{{ $loop->index }}">
                                            @foreach ($filters as $filter)
                                                <li class="flex items-center">
                                                    <button type="button"
                                                        data-filter-id="{{ $filter->id }}"
                            ...
                            <div class="filter-category hidden" id="filter-3">
                                @foreach (DB::table('filters')->where('category', 'characters')->select('id', 'title', 'filter_value')->get()->groupBy('title') as $title => $filters)
                                    <li>
                                        <div class="filter-title font-semibold mt-2">
                                            <span class="cursor-pointer" onclick="toggleDropdown('filter-values-personajes-{{ $loop->index }}', this)">
                                            {{ __($title) }} <span class="ml-2">&#9662;</span>
                                            </span>
                                        </div>
                                        <ul class="filter-options hidden pl-8 flex flex-wrap gap-2" id="filter-values-personajes-{{ $loop->index }}">
                                            @foreach ($filters as $filter)
                                                <li class="flex items-center">
                                                    <button type="button"
                                                        data-filter-id="{{ $filter->id }}"
...
const selectedFilters = new Set(@json($selectedFilters ?? []));
            ...
            filterByCategory(lastSelectedCategory);
                        ...
                        @if(auth()->user()->is_admin && !$this->creating)
                            <div><div class="border-end">               
                                <b>{{ __("Ajustos d'administrador") }}</b>
                                <div class="flex items-center mt-2">
                                    <div class="w-32">{{__('Autor')}}</div>
                                ...
                                <div class="flex items-center mt-2">
                                    <div class="w-32">{{__('Data')}}</div>
                                    
                                        <x-text-input class="block w-full" type="date" wire:model="created_at" />
                        ...
                        <div class="flex justify-between">
                            <x-primary-button>{{__('GUARDAR CANVIS')}}</x-primary-button>
                            @if ($post->id)
                                <div>
                                    <x-buttons.link-primary target="_blank" class="px-4"
                                        href="{{ route('post.show', [$post]) }}">{{ __('Veure projecte') }}
