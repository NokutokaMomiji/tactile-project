  <div>
      <div class="mb-2">
          <h2 class="text-base font-semibold leading-7 text-gray-900">{{__('Imatges')}}</h2>
      </div>
      <div>
          <label for="file" class="flex justify-center h-24" x-data="drop_file_component()">
            ...
            "
                  ...
                  <div class="text-center" wire:loading.remove wire.target="files">{{__('Fes clic o arrossega fitxers per pujar')}}</div>
                  <div class="mt-1" wire:loading.flex wire.target="files">
                      ...
                      <div>{{__('Pujant imatges...')}}</div>Ele                  </div>
              </div>
          </label>
      ...
      <div>
          @if (count($projectFiles))
              <ul class="relative grid grid-cols-4 gap-4" wire:sortable="updateImagesOrder">
                  @foreach ($projectFiles as $index => $image)
                      <li wire:sortable.item="{{ $image['id'] ?? 0 }}" wire:key="image-{{ $image['id'] ?? 0 }}"
                          class="relative bg-gray-100">
                          <a href="{{ \Storage::url($image['path']) }}" target="_blank" class="cursor-pointer">
                              <img src="{{ \Storage::url($image['path']) }}" class="w-auto h-40 mx-auto">
                          </a>
                          <button type="button"
                              class="absolute bottom-0 right-0 p-1 mb-2 mr-2 bg-gray-100 rounded-full"
                              x-on:click="if (confirm('Segur que vols eliminar la imatge?')) $wire.deleteFile({{ $index }})">
